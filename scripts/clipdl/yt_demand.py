...
check is ~101 units (the stats recorder uses some of the day too). Results are
kept for 12 hours so looking again costs nothing; every unit is counted in
yt_quota, with everything else that uses the key.
"""
...
def check(api_key, game_name, days=7, force=False, web=None):
    ...
    if not force:
        ...
        if hit:
            return hit
    if not yt_quota.can_spend(CHECK_COST):
